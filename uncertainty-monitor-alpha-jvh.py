...
# * Designed to work with Ollama + Pleias
...
# Run with: 
#     python uncertainty-monitor-alpha.py
#     python uncertainty-monitor-alpha.py "Your custom prompt here"
...
# A single background thread owns all audio output.
# Tokens drop notes into the queue; the thread plays them one at a time.
...
def _audio_worker():
    while True:
        ...
        if item is None:
            break
        freq, duration, volume, release, sample_rate = item
        try:
            n = int(sample_rate * duration)
            t = np.linspace(0, duration, n, endpoint=False)
            wave = np.sin(2 * np.pi * freq * t)
            envelope = make_envelope(n, attack=0.01, decay=0.05, sustain=0.7,
                                     release=release, sample_rate=sample_rate)
...
def play_tone(freq: float, duration=0.12, volume=0.4, release=0.08,
              sample_rate=44100):
    """
    Queue a tone for playback. A single background thread plays notes
    one at a time, preventing simultaneous streams and audio errors.
      freq     : frequency in Hz
      duration : total note length in seconds
      volume   : overall level 0.0-1.0
      release  : fade-out time in seconds (from gap_to_release)
    """
    if not AUDIO_AVAILABLE:
        return
    # Always drain any pending notes before queuing the new one.
    # This keeps audio locked to the most recent token rather than
    # playing catch-up through a backlog.
    while not _audio_queue.empty():
        try:
            ...
            break
    _audio_queue.put((freq, duration, volume, release, sample_rate))
...
def on_uncertainty_event(token: str, score: float, candidates: dict):
    ...
    Currently plays a pentatonic tone mapped to uncertainty.
    ...
    release   = gap_to_release(gap)
    play_tone(freq, release=release)
...
def stream_with_uncertainty(prompt: str):
    ...
    payload = {
        ...
        "options": {
            "temperature": 0.2,       # PleIAs recommended setting
            "repeat_penalty": 1.2,    # prevents repetitive looping
            "top_k": TOP_K_CANDIDATES,
            "stop": ["\n\n", "###"],
            "num_predict": 100,       # hard cap — model doesn't self-terminate cleanly
        },
    }
    ...
    if len(sys.argv) > 1:
        ...
        while True:
            try:
                ...
                break# ── Continuous Audio Engine ──────────────────────────────────────────────────
#
# A single audio stream runs continuously. Each token instantly updates the
# current pitch. Punctuation tokens trigger a release (brief fade to silence).
# This gives perfect sync — no queuing, no lag.
#
# Internals:
#   _current_freq   : frequency the stream is currently playing (0 = silent)
#   _releasing      : True during a punctuation fade-out
#   STREAM_CHUNK    : samples per callback (lower = more responsive)

import threading as _threading

PUNCTUATION     = set('.!?;:')
STREAM_CHUNK    = 256          # samples per audio callback (~6ms at 44100)
SAMPLE_RATE     = 44100
RELEASE_SECONDS = 0.18         # how long the fade-out lasts at punctuation

_current_freq   = 0.0          # 0 = silent
_volume         = 0.4
_phase          = 0.0          # continuous phase accumulator (prevents clicks)
_releasing      = False
_release_samples_left = 0
_audio_lock     = _threading.Lock()

def _audio_callback(outdata, frames, time_info, status):
    """Called by sounddevice for each chunk. Generates samples on the fly."""
    global _phase, _releasing, _release_samples_left, _current_freq
    ...
    if freq > 0:
        # Generate sine wave with continuous phase (no clicks between chunks)
        phases = _phase + 2 * np.pi * freq * np.arange(frames) / SAMPLE_RATE
        chunk  = np.sin(phases).astype(np.float32) * _volume
        _phase = phases[-1] % (2 * np.pi)

        if releasing and rel_left > 0:
            # Apply fade-out envelope over the release window
            fade_len = min(frames, rel_left)
            start_gain = rel_left / (RELEASE_SECONDS * SAMPLE_RATE)
            end_gain   = max(0.0, (rel_left - fade_len) / (RELEASE_SECONDS * SAMPLE_RATE))
            fade = np.linspace(start_gain, end_gain, fade_len, dtype=np.float32)
            chunk[:fade_len] *= fade
            if fade_len < frames:
                chunk[fade_len:] = 0.0
            with _audio_lock:
                _release_samples_left = max(0, rel_left - frames)
...
AUDIO_AVAILABLE = True   # stream started successfully

def set_pitch(freq: float):
    """Instantly update the pitch of the continuous tone."""
    global _current_freq, _releasing
    with _audio_lock:
        _current_freq = freq
        _releasing    = False   # cancel any ongoing release

def trigger_release():
    """Fade out over RELEASE_SECONDS (called on punctuation tokens)."""
    global _releasing, _release_samples_left
    with _audio_lock:
        if _current_freq > 0:
            _releasing            = True
            _release_samples_left = int(RELEASE_SECONDS * SAMPLE_RATE)
...
def play_tone(freq: float, **kwargs):
    """Update the continuous stream to the new frequency instantly."""
    set_pitch(freq)
